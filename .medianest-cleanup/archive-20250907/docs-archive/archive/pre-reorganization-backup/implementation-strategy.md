...
import { httpRequestDuration, httpRequestsTotal, userSessionsActive } from '../utils/metrics';
...
export class TracingService {
  ...
  static traceExternalApiCall<T>(
    ...
    return this.traceAsyncOperation(`${service}.${operation}`, apiCall, {
      'http.method': method,
      'http.url': url,
  ...
  static traceDatabaseOperation<T>(
    ...
    return this.traceAsyncOperation(`db.${operation}`, dbOperation, {
...
class PerformanceMonitor {
  ...
  private async sendCustomMetric(name: string, value: number, labels: Record<string, string>) {
    try {
...
import { mediaRequestsTotal, userSessionsActive, externalApiDuration } from '../utils/metrics';
...
export class AnalyticsService {
  ...
  static trackMediaRequest(userId: string, mediaType: string, source: string, status: string) {
  ...
  static trackError(userId: string, errorType: string, errorMessage: string, stackTrace?: string) {
...
_This implementation strategy provides MediaNest with production-ready observability while maintaining development velocity and operational simplicity._
