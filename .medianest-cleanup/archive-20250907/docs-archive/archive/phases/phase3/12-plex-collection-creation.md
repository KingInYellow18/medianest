    ...
    return (
      <div className="bg-red-900/20 border border-red-800 rounded-lg p-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-red-300">{error || 'Collection information unavailable'}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gray-800 rounded-lg p-4 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{collection.collectionTitle}</p>
        </div>

        <StatusBadge status={collection.status} />
      </div>

      {collection.status !== 'completed' && <CollectionProgress collection={collection} compact />}
      ...
      {collection.status === 'failed' && collection.error && (
        <div className="text-sm text-red-400 bg-red-900/20 rounded p-2">{collection.error}</div>
      )}
    </div>
  );
}
...
export function CollectionProgress({ collection, compact = false }: CollectionProgressProps) {
  const progress =
    collection.videoCount > 0 ? (collection.processedCount / collection.videoCount) * 100 : 0;
...
function VideoStatus({ video, index }: { video: PlexCollectionVideo; index: number }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      {video.error && (
        <span className="text-red-400 text-xs" title={video.error}>
          !
        </span>
      )}
    </div>
