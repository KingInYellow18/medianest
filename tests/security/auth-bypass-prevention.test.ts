...
describe('CRITICAL: Authentication Bypass Prevention', () => {
  ...
  describe('Cache Poisoning Prevention', () => {
    ...
    test('CRITICAL: Should use session-specific cache keys', async () => {
      ...
      const cacheKey1 = authSecurityService.generateSecureCacheKey(testUser.id, sessionId1, '192.168.1.1', 'user');
      const cacheKey2 = authSecurityService.generateSecureCacheKey(testUser.id, sessionId2, '192.168.1.1', 'user');
  ...
  describe('Suspicious Activity Detection', () => {
    ...
    test('CRITICAL: Should detect multiple IP addresses', async () => {
      const ips = ['192.168.1.1', '192.168.1.2', '192.168.1.3', '192.168.1.4', '192.168.1.5', '192.168.1.6'];
