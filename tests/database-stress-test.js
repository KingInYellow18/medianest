...
class DatabaseStressTester {
  ...
  async runConnectionStressTest() {
    ...
    try {
      for (let batch = 0; batch < batches; batch++) {
        ...
        console.log(
          `     Active connections: ${this.activeConnections.length}, Success rate: ${((connectionResults.successful / connectionResults.attempted) * 100).toFixed(2)}%`,
        );
      }
      ...
      console.log(
        `   ⚡ Testing concurrent operations on ${this.activeConnections.length} connections...`,
      );

      const operationPromises = this.activeConnections
        .slice(0, 500)
      ...
      const avgConnectionTime =
        connectionResults.connectionTimes.length > 0
          ? connectionResults.connectionTimes.reduce((a, b) => a + b, 0) /
            connectionResults.connectionTimes.length
          : 0;
      ...
      this.results.connectionStress = {
        ...
        maxConnectionTime:
          connectionResults.connectionTimes.length > 0
            ? Math.round(Math.max(...connectionResults.connectionTimes))
            : 0,
        ...
        status:
          connectionResults.failed > connectionResults.attempted * 0.1
            ? 'CRITICAL'
            : connectionResults.failed > connectionResults.attempted * 0.05
              ? 'WARNING'
              : 'GOOD',
      };
      ...
      console.log(
        `     Attempted: ${connectionResults.attempted}, Successful: ${connectionResults.successful}, Failed: ${connectionResults.failed}`,
      );
      console.log(`     Success Rate: ${this.results.connectionStress.successRate}%`);
      console.log(
        `     Peak Active: ${connectionResults.peakActiveConnections}, Avg Connect Time: ${Math.round(avgConnectionTime)}ms`,
      );
    } catch (error) {
  ...
  async runTransactionStressTest() {
    ...
    for (let i = 0; i < transactionPromises.length; i += batchSize) {
      ...
      if (i % 100 === 0) {
        console.log(
          `     Completed ${Math.min(i + batchSize, transactionPromises.length)}/${transactionCount} transactions`,
        );
      }
    }

    const avgTransactionTime =
      transactionResults.transactionTimes.length > 0
        ? transactionResults.transactionTimes.reduce((a, b) => a + b, 0) /
          transactionResults.transactionTimes.length
        : 0;
    ...
    this.results.transactionStress = {
      ...
      maxTransactionTime:
        transactionResults.transactionTimes.length > 0
          ? Math.round(Math.max(...transactionResults.transactionTimes))
          : 0,
      ...
      status:
        transactionResults.failed > transactionCount * 0.1
          ? 'CRITICAL'
          : transactionResults.failed > transactionCount * 0.05
            ? 'WARNING'
            : 'GOOD',
    };
    ...
    console.log(
      `     Successful: ${transactionResults.successful}, Failed: ${transactionResults.failed}, Deadlocked: ${transactionResults.deadlocked}`,
    );
    console.log(
      `     Success Rate: ${this.results.transactionStress.successRate}%, Deadlock Rate: ${this.results.transactionStress.deadlockRate}%`,
    );
  ...
  async runQueryThroughputStress() {
    ...
    const queries = [
      ...
      {
        sql: 'SELECT * FROM media_requests WHERE created_at > $1 ORDER BY created_at DESC LIMIT 10',
        params: [new Date(Date.now() - 24 * 60 * 60 * 1000)],
      },
      {
        sql: 'SELECT service_name, status, response_time_ms FROM service_status WHERE last_check_at > $1',
        params: [new Date(Date.now() - 60 * 60 * 1000)],
      },
      { sql: 'SELECT COUNT(*) FROM session_tokens WHERE expires_at > $1', params: [new Date()] },
      {
        sql: 'SELECT user_id, endpoint, request_count FROM rate_limits WHERE window_start > $1 LIMIT 20',
        params: [new Date(Date.now() - 15 * 60 * 1000)],
      },
      {
        sql: 'SELECT * FROM youtube_downloads WHERE status = $1 AND created_at > $2 LIMIT 5',
        params: ['completed', new Date(Date.now() - 24 * 60 * 60 * 1000)],
      },
    ];
    ...
    const avgQueryTime =
      throughputResults.queryTimes.length > 0
        ? throughputResults.queryTimes.reduce((a, b) => a + b, 0) /
          throughputResults.queryTimes.length
        : 0;
    ...
    this.results.queryThroughputStress = {
      ...
      successRate: Math.round(
        (throughputResults.successfulQueries / throughputResults.totalQueries) * 100,
      ),
      workerResults,
    ...
    console.log(
      `     Total Queries: ${throughputResults.totalQueries}, Success Rate: ${this.results.queryThroughputStress.successRate}%`,
    );
    console.log(`     Throughput: ${queriesPerSecond} queries/sec`);
    console.log(
      `     Avg Time: ${avgQueryTime.toFixed(2)}ms, P95: ${p95Time.toFixed(2)}ms, P99: ${p99Time.toFixed(2)}ms`,
    );
  }
  ...
  async runDeadlockHandlingTest() {
    ...
    const deadlockPromises = Array.from({ length: deadlockTestCount }, async (_, i) => {
      ...
      try {
        ...
        await Promise.all([conn1.query('BEGIN'), conn2.query('BEGIN')]);
        ...
        const promise1 = conn1.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [
          `deadlock_user_2_${i}`,
        ]);
        const promise2 = conn2.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [
          `deadlock_user_1_${i}`,
        ]);
    ...
    if (deadlockResults.resolutionTimes.length > 0) {
      deadlockResults.averageResolutionTime = Math.round(
        deadlockResults.resolutionTimes.reduce((a, b) => a + b, 0) /
          deadlockResults.resolutionTimes.length,
      );
      deadlockResults.maxResolutionTime = Math.round(Math.max(...deadlockResults.resolutionTimes));
    }

    deadlockResults.deadlockDetectionRate = Math.round(
      (deadlockResults.deadlocksDetected / deadlockTestCount) * 100,
    );
    deadlockResults.status =
      deadlockResults.deadlocksResolved > 0 && deadlockResults.averageResolutionTime < 1000
        ? 'GOOD'
        : 'WARNING';
    ...
    console.log(
      `     Deadlocks Detected: ${deadlockResults.deadlocksDetected}/${deadlockTestCount} (${deadlockResults.deadlockDetectionRate}%)`,
    );
  ...
  async runRedisStressTest() {
    ...
    redisResults.successRate = Math.round(
      (redisResults.successfulOperations / redisResults.totalOperations) * 100,
    );
    redisResults.status =
      redisResults.avgOperationTime < 5 && redisResults.successRate > 95
        ? 'GOOD'
        : redisResults.avgOperationTime < 10 && redisResults.successRate > 90
          ? 'WARNING'
          : 'CRITICAL';
    ...
    console.log(
      `     Total Operations: ${redisResults.totalOperations}, Success Rate: ${redisResults.successRate}%`,
    );
  ...
  async runRecoveryTest() {
    ...
    try {
      ...
      await Promise.all(
        connectionsToClose.map(async ({ client, connection }) => {
          try {
            connection.release();
            await client.end();
          } catch (error) {
      ...
      console.log(
        `     Recovery: ${newConnections.length}/25 connections recovered in ${Math.round(recoveryTime)}ms`,
      );
    } catch (error) {
    ...
    try {
      ...
      await Promise.all(
        clientsToDisconnect.map(async ({ client }) => {
          try {
      ...
      console.log(
        `     Redis Recovery: ${newRedisClients.length}/15 clients recovered in ${Math.round(redisRecoveryTime)}ms`,
      );
    } catch (error) {
  ...
  async analyzePerformanceDegradation() {
    ...
    const baselineAvg =
      degradationResults.baselineQueries.reduce((a, b) => a + b, 0) /
      degradationResults.baselineQueries.length;
    const stressAvg =
      degradationResults.stressQueries.reduce((a, b) => a + b, 0) /
      degradationResults.stressQueries.length;

    degradationResults.degradationPercentage = Math.round(
      ((stressAvg - baselineAvg) / baselineAvg) * 100,
    );
    ...
    const recoveryAvg =
      degradationResults.recoveryQueries.reduce((a, b) => a + b, 0) /
      degradationResults.recoveryQueries.length;
    degradationResults.recoveryPercentage = Math.round(
      ((recoveryAvg - baselineAvg) / baselineAvg) * 100,
    );
    ...
    degradationResults.status =
      degradationResults.degradationPercentage < 50
        ? 'GOOD'
        : degradationResults.degradationPercentage < 100
          ? 'WARNING'
          : 'CRITICAL';
    ...
    console.log(
      `     Under Stress: ${stressAvg.toFixed(2)}ms (${degradationResults.degradationPercentage}% degradation)`,
    );
    console.log(
      `     After Recovery: ${recoveryAvg.toFixed(2)}ms (${degradationResults.recoveryPercentage}% from baseline)`,
    );
  }
  ...
  calculateStressTestScore() {
    ...
    if (this.results.queryThroughputStress.successRate) {
      const throughputScore = Math.min(
        20,
        (this.results.queryThroughputStress.queriesPerSecond / 1000) * 10 +
    ...
    const status =
      score >= 80 ? 'EXCELLENT' : score >= 70 ? 'GOOD' : score >= 60 ? 'WARNING' : 'CRITICAL';
  ...
  async cleanup() {
    ...
    try {
      ...
      await connection.query(
        `UPDATE service_status SET last_check_at = NOW() WHERE service_name LIKE 'stress_service_%'`,
      );
...
if (require.main === module) {
  ...
  console.log(
    '⚠️  WARNING: This is a HIGH INTENSITY stress test that may impact system performance',
  );
