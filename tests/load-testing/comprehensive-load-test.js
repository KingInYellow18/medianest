...
class MediaNestLoadTester {
  ...
  recordMetrics(type, responseTime, success) {
    ...
    this.metrics.responseTypes[type] = this.metrics.responseTypes[type] || {
      count: 0,
      totalTime: 0,
    };
  ...
  generateReport() {
    ...
    const avgResponseTime =
      this.metrics.responseTime.length > 0
        ? this.metrics.responseTime.reduce((sum, r) => sum + r.time, 0) /
    ...
    const report = {
      ...
      databaseMetrics: {
        averageQueryTime:
          this.metrics.databaseMetrics.queryTimes.length > 0
      ...
      redisMetrics: {
        cacheHitRate:
          this.metrics.redisMetrics.cacheHits + this.metrics.redisMetrics.cacheMisses > 0
            ...
            : 'N/A',
        averageResponseTime:
          this.metrics.redisMetrics.responseTimes.length > 0
...
async function main() {
  ...
  try {
    ...
    try {
      execSync(
        `npx claude-flow@alpha hooks post-edit --file "load-test-results.json" --memory-key "MEDIANEST_PROD_VALIDATION/performance_load"`,
        {
