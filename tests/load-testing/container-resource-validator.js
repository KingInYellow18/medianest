...
class ContainerResourceValidator {
  ...
  async validateResourceLimits() {
    ...
    for (const [service, containerName] of Object.entries(this.config.containers)) {
      try {
        ...
        console.log(
          `📊 ${containerName} limits: Memory=${this.formatBytes(memoryLimit)}, CPU=${limits.cpu ? `${limits.cpu.toFixed(2)} cores` : 'unlimited'}`,
        );
      } catch (error) {
  ...
  generateReport() {
    const report = {
      ...
      systemSummary: {
        peakCPU:
          this.metrics.systemMetrics.totalCPU.length > 0
            ? Math.max(...this.metrics.systemMetrics.totalCPU.map((c) => c.percentage))
            : 0,
        peakMemory:
          this.metrics.systemMetrics.totalMemory.length > 0
    ...
    for (const [containerName, metrics] of Object.entries(this.metrics.containers)) {
      ...
      const memAvg =
        metrics.memoryUsage.length > 0
          ? metrics.memoryUsage.reduce((sum, m) => sum + m.percentage, 0) /
            metrics.memoryUsage.length
          : 0;

      const peakCPU =
        metrics.cpuUsage.length > 0 ? Math.max(...metrics.cpuUsage.map((c) => c.percentage)) : 0;
