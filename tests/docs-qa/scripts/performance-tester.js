...
class PerformanceTester {
  ...
  async testBuildPerformance() {
    ...
    try {
      ...
      if (await fs.pathExists(buildDir)) {
        ...
        // Check build size
        if (stats.totalSize > 50 * 1024 * 1024) { // 50MB threshold
          test.issues.push(`Large build size: ${(stats.totalSize / 1024 / 1024).toFixed(2)}MB`);
          test.status = 'WARNING';
        }
        
        // Check build time
        if (buildTime > 30000) { // 30 second threshold
  ...
  async testPagePerformance(pageInfo) {
    ...
    try {
      ...
      const statusColor = test.status === 'PASS' ? 'green' : test.status === 'WARNING' ? 'yellow' : 'red';
      const statusIcon = test.status === 'PASS' ? '✅' : test.status === 'WARNING' ? '⚠️' : '❌';
      
      console.log(chalk[statusColor](`${statusIcon} ${pageInfo.name} - Performance: ${test.lighthouse.performanceScore}, Issues: ${test.issues.length}`));
  ...
  evaluatePerformance(test) {
    ...
    if (test.lighthouse.performanceScore < this.performanceThresholds.performanceScore) {
      issues.push(`Low performance score: ${test.lighthouse.performanceScore} (threshold: ${this.performanceThresholds.performanceScore})`);
      status = test.critical ? 'FAIL' : 'WARNING';
    }
    
    if (test.lighthouse.accessibilityScore < this.performanceThresholds.accessibilityScore) {
      issues.push(`Low accessibility score: ${test.lighthouse.accessibilityScore} (threshold: ${this.performanceThresholds.accessibilityScore})`);
      status = 'WARNING';
    }
    
    if (test.lighthouse.seoScore < this.performanceThresholds.seoScore) {
      issues.push(`Low SEO score: ${test.lighthouse.seoScore} (threshold: ${this.performanceThresholds.seoScore})`);
      status = 'WARNING';
    }
    
    // Check Core Web Vitals
    if (test.lighthouse.metrics.largestContentfulPaint > this.performanceThresholds.largestContentfulPaint) {
      issues.push(`Slow LCP: ${Math.round(test.lighthouse.metrics.largestContentfulPaint)}ms (threshold: ${this.performanceThresholds.largestContentfulPaint}ms)`);
      status = test.critical ? 'FAIL' : 'WARNING';
    }
    
    if (test.lighthouse.metrics.cumulativeLayoutShift > this.performanceThresholds.cumulativeLayoutShift) {
      issues.push(`High CLS: ${test.lighthouse.metrics.cumulativeLayoutShift.toFixed(3)} (threshold: ${this.performanceThresholds.cumulativeLayoutShift})`);
      status = 'WARNING';
    }
    
    // Check custom metrics
    if (test.customMetrics.loadTime > 5000) { // 5 second threshold
  ...
  generateReport() {
    ...
    if (testCount > 0) {
      this.results.summary.averagePerformanceScore = Math.round(this.results.summary.averagePerformanceScore / testCount);
      this.results.summary.averageAccessibilityScore = Math.round(this.results.summary.averageAccessibilityScore / testCount);
      this.results.summary.averageBestPracticesScore = Math.round(this.results.summary.averageBestPracticesScore / testCount);
      this.results.summary.averageSeoScore = Math.round(this.results.summary.averageSeoScore / testCount);
      this.results.summary.averageLoadTime = Math.round(this.results.summary.averageLoadTime / testCount);
    }
    ...
    console.log(chalk.white(`Average Performance Score: ${report.summary.averagePerformanceScore}`));
    console.log(chalk.white(`Average Accessibility Score: ${report.summary.averageAccessibilityScore}`));
    console.log(chalk.white(`Average Load Time: ${(report.summary.averageLoadTime / 1000).toFixed(2)}s`));
  ...
  generateRecommendations() {
    ...
    const allOpportunities = this.results.tests.flatMap(test => test.lighthouse.opportunities || []);
    ...
    if (opportunityGroups['unused-css-rules'] > 1) {
      recommendations.push('Remove unused CSS rules to reduce file size and improve loading performance');
    }
    ...
    if (opportunityGroups['render-blocking-resources'] > 1) {
      recommendations.push('Optimize CSS and JavaScript delivery to reduce render-blocking resources');
    }
    ...
    const slowPages = this.results.tests.filter(test => 
      test.lighthouse.performanceScore < this.performanceThresholds.performanceScore
    );
