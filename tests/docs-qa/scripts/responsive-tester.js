...
class ResponsiveTester {
  ...
  async testBrowser(browserName) {
    ...
    const browserLib = browserName === 'chromium' ? chromium : browserName === 'firefox' ? firefox : webkit;
  ...
  async testPage(page, browserName, viewport, pagePath) {
    ...
    try {
      ...
      if (scrollWidth > clientWidth + 1) { // Allow 1px tolerance
        test.issues.push({
      ...
      if (viewport.width <= 768) {
        const mobileMenuExists = await page.locator('.hamburger, .mobile-menu-toggle, [aria-label*="menu"]').count() > 0;
        if (!mobileMenuExists) {
          test.issues.push({
            type: 'mobile-menu',
            severity: 'warning',
  ...
  getUserAgent(device) {
    const userAgents = {
      'iPhone SE': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.0 Mobile/15E148 Safari/604.1',
      ...
      'Desktop Small': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
      'Desktop Large': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    };
  ...
  analyzeResults() {
    ...
    if (analysis.commonIssues['horizontal-scroll'] > 0) {
      analysis.recommendations.push('Implement responsive CSS units (%, vw, rem) instead of fixed pixel widths');
    }
    
    if (analysis.commonIssues['font-size'] > 0) {
      analysis.recommendations.push('Increase font sizes for better readability, especially on mobile devices');
    }
    
    if (analysis.commonIssues['touch-target'] > 0) {
      analysis.recommendations.push('Ensure all interactive elements meet the minimum 44x44px touch target size');
    }
    
    if (analysis.commonIssues['layout-shift'] > 0) {
      analysis.recommendations.push('Optimize layout stability by reserving space for dynamic content');
    }
