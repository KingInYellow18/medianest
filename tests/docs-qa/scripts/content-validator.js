...
class ContentValidator {
  ...
  async validateMarkdownFile(relativePath) {
    ...
    try {
      ...
      const statusColor =
        fileResult.status === 'PASS' ? 'green' : fileResult.status === 'WARNING' ? 'yellow' : 'red';
      const statusIcon =
        fileResult.status === 'PASS' ? '✅' : fileResult.status === 'WARNING' ? '⚠️' : '❌';

      console.log(
        chalk[statusColor](`${statusIcon} ${relativePath} - ${fileResult.issues.length} issues`),
      );
    } catch (error) {
  ...
  async checkContentCompleteness() {
    ...
    try {
      ...
      const orphanedFiles = allMarkdownFiles.filter(
        (file) => !referencedFiles.includes(file) && file !== 'index.md',
      );
