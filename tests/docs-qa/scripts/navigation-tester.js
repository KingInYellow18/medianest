...
class NavigationTester {
  ...
  async testMainNavigation() {
    ...
    try {
      ...
      for (const [index, link] of navLinks.entries()) {
        ...
        try {
          ...
          const linkResponse = await page.goto(
            href.startsWith('http') ? href : `${this.siteUrl}${href.startsWith('/') ? href : '/' + href}`,
          ...
          if (linkResponse && linkResponse.ok()) {
            ...
            linkTests.push({ text, href, status: 'broken', error: `HTTP ${linkResponse?.status()}` });
          }
      ...
      test.metrics.brokenLinks = linkTests.filter(l => l.status === 'broken' || l.status === 'error').length;
      ...
      const hasActiveIndicator = await page.locator('nav .active, nav [aria-current], .navigation .current').count() > 0;
  ...
  async testSearchFunctionality() {
      ...
      if (searchInputCount === 0) {
        ...
        for (const query of searchQueries) {
          try {
            ...
            const resultsVisible = await page.locator('.search-results, .suggestions, [role="listbox"]').isVisible()
              .catch(() => false);
            
            const resultCount = await page.locator('.search-results li, .suggestions li, [role="option"]').count()
              .catch(() => 0);
    ...
    const statusColor = test.status === 'PASS' ? 'green' : test.status === 'WARNING' ? 'yellow' : 'red';
    const statusIcon = test.status === 'PASS' ? '✅' : test.status === 'WARNING' ? '⚠️' : '❌';
    console.log(chalk[statusColor](`${statusIcon} Search Functionality - ${test.issues.length} issues`));
  }
    ...
    for (const viewport of this.viewports) {
      ...
      try {
        ...
        if (viewport.width <= 768) {
          ...
          const mobileToggle = page.locator('.hamburger, .mobile-menu-toggle, [aria-label*="menu" i]');
    ...
    try {
      ...
      for (const testPage of testPages) {
        try {
          ...
          if (hasBreadcrumbs) {
            const breadcrumbLinks = await page.locator('.breadcrumb a, [aria-label="breadcrumb"] a').count();
  ...
  async testTableOfContents() {
    ...
    try {
      ...
      for (const testPage of testPages) {
        ...
        const toc = page.locator('.toc, .table-of-contents, [role="navigation"][aria-label*="contents" i]');
    ...
    const statusColor = test.status === 'PASS' ? 'green' : test.status === 'WARNING' ? 'yellow' : 'red';
    const statusIcon = test.status === 'PASS' ? '✅' : test.status === 'WARNING' ? '⚠️' : '❌';
    console.log(chalk[statusColor](`${statusIcon} Table of Contents - ${test.issues.length} issues`));
  }
      ...
      if (!hasFooter) {
        ...
        for (const [index, link] of footerLinks.entries()) {
          ...
          if (href.startsWith('http')) {
            ...
            // Test internal links
            try {
              const fullUrl = href.startsWith('/') ? `${this.siteUrl}${href}` : `${this.siteUrl}/${href}`;
              const response = await page.goto(fullUrl, { timeout: 5000 });
              
              if (response && response.ok()) {
                linkTests.push({ text, href, status: 'working', type: 'internal' });
              } else {
                linkTests.push({ text, href, status: 'broken', type: 'internal', statusCode: response?.status() });
              }
            } catch (error) {
              linkTests.push({ text, href, status: 'error', type: 'internal', error: error.message });
            }
          }
        }
        ...
        const brokenLinks = linkTests.filter(l => l.status === 'broken' || l.status === 'error').length;
