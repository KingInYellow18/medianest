...
class AccessibilityTester {
  ...
  async testPage(browser, pageInfo) {
    ...
    try {
      ...
      if (test.violations.length > 0) {
        const criticalOrSerious = test.violations.some(v => ['critical', 'serious'].includes(v.impact));
      ...
      console.log(chalk[statusColor](`${statusIcon} ${pageInfo.name} - ${violationCount} violations, ${test.passes.length} passed rules`));
  ...
  async testKeyboardNavigation(page) {
    ...
    try {
      ...
      if (focusedCount < Math.min(focusableElements.length, 10) * 0.8) {
        test.issues.push(`Only ${focusedCount} of ${Math.min(focusableElements.length, 10)} elements show visible focus`);
        test.status = 'WARNING';
      }
  ...
  async testScreenReaderCompat(page) {
    ...
    try {
      ...
      const missingLandmarks = requiredLandmarks.filter(landmark => 
        !landmarks.some(l => l.toLowerCase().includes(landmark))
      );
  ...
  async testColorContrast(page) {
    ...
    try {
      ...
      const textElements = await page.$$eval('p, h1, h2, h3, h4, h5, h6, a, button, span, div', elements => {
  ...
  generateRecommendations() {
    ...
    if (this.results.summary.criticalViolations > 0) {
      recommendations.push('Address all critical violations immediately - these prevent basic accessibility');
    }
    ...
    if (violationTypes['color-contrast']) {
      recommendations.push('Improve color contrast ratios to meet WCAG standards (4.5:1 for normal text)');
    }
...
if (require.main === module) {
  ...
  tester.runTests()
    ...
    .then(() => {
      const hasFailures = tester.results.summary.criticalViolations > 0 || 
