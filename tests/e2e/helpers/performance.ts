  ...
  async generatePerformanceReport(url: string): Promise<{
    ...
    if (pageMetrics.loadTime > 3000) {
      recommendations.push(
        'Page load time exceeds 3 seconds. Consider optimizing critical resources.',
      );
    }
    ...
    if (resourceMetrics.slowResources.length > 0) {
      recommendations.push(
        `${resourceMetrics.slowResources.length} resources loading slowly. Review network requests.`,
      );
    }
