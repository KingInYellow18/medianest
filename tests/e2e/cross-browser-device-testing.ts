...
export class CrossBrowserDeviceTester {
  ...
  private async executeDeviceScenario(
    ...
    const context = await browser.newContext({
      ...
      recordVideo: { dir: `test-results/cross-browser/${browserConfig.name}-${deviceConfig.name}/` }
    });
    ...
    try {
      ...
      const scenarioResult = await this.executeScenario(page, scenario, deviceConfig, browserConfig);
   ...
   * Setup device-specific monitoring
   */
  private async setupDeviceMonitoring(page: Page, deviceConfig: DeviceTestConfiguration): Promise<void> {
   ...
   */
  private async testUserAuthenticationFlow(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    ...
    try {
      ...
      if (deviceConfig.isMobile) {
        ...
        const mobileLoginVisible = await page.locator('[data-testid="mobile-login-options"]').isVisible();
        features.push({
      ...
      return {
        ...
        performance: { pageLoadTime: 0, firstContentfulPaint: 0, largestContentfulPaint: 0, memoryUsage: 0, networkRequests: 0, jsErrors: 0 },
        screenshots,
        errors
      };
    }
   ...
   */
  private async testMediaUploadAndProcessing(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      if (deviceConfig.isMobile) {
        const cameraButtonVisible = await page.locator('[data-testid="camera-upload-button"]').isVisible();
        features.push({
   ...
   */
  private async testResponsiveDesign(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      for (const breakpoint of breakpoints) {
        ...
        const correctMenuDisplayed = breakpoint.width < 768 ? mobileMenuVisible : desktopMenuVisible;
   ...
   */
  private async testPWAFeatures(page: Page, deviceConfig: DeviceTestConfiguration, browserConfig: BrowserTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      return {
        success: features.filter(f => !f.feature.includes('install_prompt')).every(f => f.supported),
        features,
        accessibility,
        performance,
        screenshots,
   ...
   */
  private async testAccessibilityCompliance(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
   ...
   */
  private async testTouchAndGestureSupport(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
   ...
   * Test offline functionality
   */
  private async testOfflineFunctionality(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      return {
        success: features.filter(f => f.feature !== 'offline_indicator').every(f => f.supported),
        features,
        accessibility,
        performance,
        screenshots,
   ...
   */
  private async testPerformanceOptimization(page: Page, deviceConfig: DeviceTestConfiguration): Promise<any> {
    const features: FeatureTestResult[] = [];
    const screenshots: Buffer[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      return {
        success: features.filter(f => !['webp_images', 'gzip_compression'].includes(f.feature)).every(f => f.supported),
        features,
        accessibility,
        performance,
        screenshots,
...
${Array.from(browserResults.entries()).map(([browser, browserTests]) => `
### ${browser}
...
${Array.from(deviceResults.entries()).map(([device, deviceTests]) => `
### ${device}
...
${this.generateCrossBrowserRecommendations(results)}
  ...
  private generateCrossBrowserRecommendations(results: CrossBrowserTestResult[]): string {
    ...
    const mobileIssues = results.filter(r => r.device.includes('iPhone') || r.device.includes('Pixel') || r.device.includes('Galaxy')).filter(r => !r.success);
...
export { CrossBrowserDeviceTester, CrossBrowserTestResult, DeviceTestConfiguration, BrowserTestConfiguration };