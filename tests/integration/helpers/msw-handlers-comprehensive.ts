...
const plexHandlers = [
  ...
  // Plex Collections
  http.get(
    'http://plex.local:32400/library/sections/:sectionId/collections',
    ({ params, request }) => {
      const token = new URL(request.url).searchParams.get('X-Plex-Token');
      if (!token) return new HttpResponse(null, { status: 401 });
...
const overseerrHandlers = [
  ...
  http.post(
    'http://overseerr.local:5055/api/v1/request/:requestId/approve',
    ({ params, request }) => {
      const apiKey = request.headers.get('X-Api-Key');
...
const youtubeHandlers = [
  ...
  http.get('http://localhost:8080/api/download/:jobId/status', ({ params }) => {
    ...
    return HttpResponse.json({
      ...
      status: randomState,
      progress:
        randomState === 'downloading'
          ? Math.floor(Math.random() * 100)
          : randomState === 'completed'
            ? 100
            : 0,
      downloadSpeed:
        randomState === 'downloading' ? `${(Math.random() * 10 + 1).toFixed(1)} MB/s` : null,
...
export { plexHandlers, overseerrHandlers, youtubeHandlers, uptimeKumaHandlers, errorHandlers };
