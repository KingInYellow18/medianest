...
export class ServiceMockManager {
  ...
  simulateServiceFailure(
    serviceName: string,
    errorType: 'timeout' | 'unavailable' | 'error' = 'unavailable',
  ) {
    const failureMock = {
      healthCheck: () => {
        switch (errorType) {
          case 'timeout':
            return new Promise((resolve) =>
              setTimeout(() => resolve({ healthy: false, error: 'Timeout' }), 10000),
            );
          case 'unavailable':
...
export class CacheTestManager {
  ...
  async setupCacheScenario(scenario: 'cold' | 'warm' | 'hot' | 'invalid') {
    const cacheKeys = ['service:health:*', 'user:session:*', 'media:search:*', 'api:rate-limit:*'];
    ...
    switch (scenario) {
      ...
      case 'warm':
        ...
        await this.redis.setex(
          'service:health:plex',
          300,
          JSON.stringify({ healthy: true, lastChecked: Date.now() }),
        );
        await this.redis.setex(
          'service:health:overseerr',
          300,
          JSON.stringify({ healthy: true, lastChecked: Date.now() }),
        );
        break;
      ...
      case 'invalid':
        ...
        await this.redis.setex(
          'service:health:plex',
          1,
          JSON.stringify({ healthy: true, lastChecked: Date.now() - 10000 }),
        );
  ...
  async validateCacheInvalidation(
    ...
    expectedInvalidatedKeys: string[],
  ) {
...
export class IntegrationTestOrchestrator {
  ...
  getServiceMockManager() {
    return this.serviceMockManager;
  }
  getRealTimeManager() {
    return this.realTimeManager;
  }
  getLoadTestManager() {
    return this.loadTestManager;
  }
  getCacheManager() {
    return this.cacheManager;
  }
  getErrorScenarioManager() {
    return this.errorScenarioManager;
  }
