...
class NetworkPerformanceMonitor {
    ...
    for (const [serviceId, service] of Object.entries(this.config.services)) {
      if (service.url) {
        try {
          ...
          if (result.success) {
            console.log(`   ✅ ${service.name}: ${result.responseTime.toFixed(2)}ms (${result.statusCode})`);
          } else {
  ...
  async measureExternalAPITimes() {
    ...
    const externalAPIs = {
      ...
      cloudflare_cdn: { url: 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js', name: 'CDN Test' },
  ...
  generateInsights(results) {
    ...
    if (responseTimes.length > 0) {
      ...
      if (avgResponseTime > 1000) {
        insights.recommendations.push('High average response time detected - investigate performance bottlenecks');
      }
    }
  ...
  async runValidation() {
    ...
    try {
      ...
      console.log(`🌐 Service Availability: ${insights.performance_metrics.service_availability || 0}%`);
      
      if (insights.performance_metrics.avg_response_time) {
        console.log(`📊 Average Response Time: ${insights.performance_metrics.avg_response_time.toFixed(2)}ms`);
      }
  ...
  async storeForCoordination() {
    try {
      ...
      console.log(`   • Service Availability: ${this.memoryStore.key_metrics.service_availability || 0}%`);
      console.log(`   • Validation Duration: ${(this.memoryStore.key_metrics.validation_duration || 0) / 1000}s`);
      console.log(`   • Recommendations: ${this.memoryStore.key_metrics.recommendations_count || 0}`);
