...
class NetworkValidationSuite {
  ...
  async runThroughputAnalysis() {
    ...
    try {
      ...
      console.log(`   ✅ Throughput analysis completed in ${((testEnd - testStart) / 1000).toFixed(2)}s`);
  ...
  async runProxyPerformanceTest() {
    ...
    try {
      ...
      console.log(`   ✅ Proxy performance test completed in ${((testEnd - testStart) / 1000).toFixed(2)}s`);
  ...
  async runContainerNetworkTest() {
    ...
    try {
      ...
      if (result.success) {
        console.log(`   ✅ Container network test completed in ${((testEnd - testStart) / 1000).toFixed(2)}s`);
      } else {
  ...
  async runBandwidthTest() {
    ...
    try {
      ...
      console.log(`   ✅ Bandwidth test completed in ${((testEnd - testStart) / 1000).toFixed(2)}s`);
  ...
  generateSummary() {
    ...
    const completedTests = Object.values(this.results.tests).filter(test => test.status === 'completed').length;
    const failedTests = Object.values(this.results.tests).filter(test => test.status === 'failed').length;
    const totalDuration = Object.values(this.results.tests).reduce((sum, test) => sum + (test.duration || 0), 0);
    
    // Service availability summary
    const availableServices = Object.values(this.results.service_availability || {})
    ...
    if (this.results.tests.throughput_analysis?.results?.throughput) {
      const throughputResults = Object.values(this.results.tests.throughput_analysis.results.throughput);
      const avgSuccessRate = throughputResults.reduce((sum, r) => sum + (r.successRate || 0), 0) / throughputResults.length;
      const avgLatency = throughputResults.reduce((sum, r) => sum + (r.avgLatency || 0), 0) / throughputResults.length;
    ...
    if (this.results.tests.proxy_performance?.results?.load_balancing) {
      const proxyResults = Object.values(this.results.tests.proxy_performance.results.load_balancing);
    ...
    console.log('==========================================');
    console.log(`📊 Tests: ${completedTests}/${Object.keys(this.results.tests).length} completed (${this.results.summary.test_execution.success_rate}% success rate)`);
    console.log(`🌐 Services: ${availableServices}/${totalServices} available (${this.results.summary.service_availability.availability_rate}%)`);
    console.log(`⏱️  Total Duration: ${this.results.summary.test_execution.total_duration_seconds}s`);
  ...
  async storeInMemory() {
    const memoryData = {
      ...
      key_metrics: {
        tests_completed: Object.values(this.results.tests).filter(t => t.status === 'completed').length,
        services_available: Object.values(this.results.service_availability || {}).filter(s => s.available).length,
    ...
    try {
      const { spawn } = require('child_process');
      const storeProcess = spawn('npx', ['claude-flow@alpha', 'hooks', 'post-edit', 
        '--memory-key', 'swarm/network-validator/results',
  ...
  // Save final report
  async saveReport() {
    const reportPath = path.join(this.config.outputDir, `network-validation-suite-${Date.now()}.json`);
