  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      if (response.status === 200) {
        addResult('HEALTH_ENDPOINTS', endpoint.description, 'PASS', 
                 `Responded with status ${response.status}`, 
                 { path: endpoint.path, status: response.status }, duration);
      } else {
        addResult('HEALTH_ENDPOINTS', endpoint.description, 'WARNING',
                 `Responded with status ${response.status} (expected 200)`,
                 { path: endpoint.path, status: response.status }, duration);
      }
    } catch (error) {
      const duration = performance.now() - startTime;
      addResult('HEALTH_ENDPOINTS', endpoint.description, 'FAIL',
               `Failed: ${error.message}`,
               { path: endpoint.path, error: error.message }, duration);
    }
  }
}
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        addResult('METRICS_ENDPOINTS', endpoint.description, status,
                 contentCheck ? `Valid response with expected content` : 
                               `Response received but content validation failed`,
                 { path: endpoint.path, status: response.status, ...contentDetails }, duration);
                 
      } else if (response.status === 404) {
        addResult('METRICS_ENDPOINTS', endpoint.description, 'SKIP',
                 `Endpoint not found (404) - may not be implemented`,
                 { path: endpoint.path, status: response.status }, duration);
      } else {
        addResult('METRICS_ENDPOINTS', endpoint.description, 'WARNING',
                 `Responded with status ${response.status}`,
                 { path: endpoint.path, status: response.status }, duration);
      }
    } catch (error) {
      const duration = performance.now() - startTime;
      addResult('METRICS_ENDPOINTS', endpoint.description, 'FAIL',
               `Failed: ${error.message}`,
               { path: endpoint.path, error: error.message }, duration);
    }
  }
...
async function testApplicationPerformanceMonitoring() {
  ...
  try {
    ...
    addResult('APM', 'Request Performance Tracking', 'PASS',
    ...
    if (metricsResponse.status === 200) {
      ...
      addResult('APM', 'APM Metrics Collection', 
    ...
    addResult('APM', 'Application Performance Monitoring', 'FAIL',
...
async function testDatabaseAndRedisHealth() {
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      addResult('DATABASE_MONITORING', 'Database Connectivity Health Check',
               dbHealthy ? 'PASS' : 'WARNING',
               dbHealthy ? `Database connected (${healthData.database?.responseTime})` :
      ...
      addResult('DATABASE_MONITORING', 'Redis Connectivity Health Check',
               redisHealthy ? 'PASS' : 'WARNING', 
               redisHealthy ? `Redis connected (${healthData.redis?.responseTime})` :
      ...
      addResult('DATABASE_MONITORING', 'Health Metrics Endpoint', 'FAIL',
               'Health metrics endpoint not responding properly',
               { status: response.status });
    }
  } catch (error) {
    addResult('DATABASE_MONITORING', 'Database and Redis Health Monitoring', 'FAIL',
...
async function testMemoryAndSystemMonitoring() {
  ...
  try {
    ...
    if (metricsResponse.status === 200) {
      ...
      addResult('MEMORY_MONITORING', 'Node.js Memory Metrics Collection',
    ...
    if (healthResponse.status === 200 && healthResponse.data) {
      const hasMemoryInfo = healthResponse.data.memory &&
        healthResponse.data.memory.rss &&
        healthResponse.data.memory.heapUsed;
        
      addResult('MEMORY_MONITORING', 'Memory Usage Health Reporting',
               hasMemoryInfo ? 'PASS' : 'WARNING',
               hasMemoryInfo ? 'Memory usage included in health metrics' :
                              'Memory usage not found in health metrics',
               { memoryReporting: hasMemoryInfo });
    }
    
  } catch (error) {
    addResult('MEMORY_MONITORING', 'Memory and System Monitoring', 'FAIL',
...
async function testBusinessMetrics() {
  ...
  try {
    ...
    if (metricsResponse.status === 200) {
      ...
      addResult('BUSINESS_METRICS', 'Custom Business Metrics',
      ...
      addResult('BUSINESS_METRICS', 'Authentication and Request Tracking',
               authMetrics ? 'PASS' : 'WARNING',
               authMetrics ? 'HTTP request metrics available for auth tracking' :
                            'No HTTP request metrics found',
               { authTracking: authMetrics });
    }
    
  } catch (error) {
    addResult('BUSINESS_METRICS', 'Business Metrics Collection', 'FAIL',
...
function generateReport() {
  ...
  log(overallScore >= 80 ? 'green' : overallScore >= 60 ? 'yellow' : 'red',
    ...
    if (cat.score < 70) {
      switch (cat.category) {
        ...
        case 'METRICS_ENDPOINTS': 
          recommendations.push('📊 Implement missing metrics endpoints for comprehensive monitoring');
          break;
        case 'APM':
