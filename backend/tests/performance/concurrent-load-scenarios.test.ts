...
describe('Concurrent User Handling and Load Scenarios', () => {
  ...
  beforeAll(async () => {
    ...
    const userCreationPromises = Array(100)
      .fill(null)
      .map((_, index) => authHelper.createTestUser(`loadtest${index}@medianest.test`));
  ...
  describe('Burst and Spike Testing', () => {
    test('should handle sudden traffic spikes', async () => {
      ...
      const spike = await executeLoadScenario('spike-80', 80, 15000, 'browsing', 80, 0.8);
      ...
      expect(spikeSuccessRate).toBeGreaterThan(0.7); // Should handle spike reasonably
      expect(recoverySuccessRate).toBeGreaterThan(0.9); // Should recover well
    ...
    test('should meet overall load handling requirements', async () => {
      const loadSummary = {
        ...
        scenariosByLoad: {
          ...
          peak: loadResults.filter((r) => r.concurrentUsers > 20 && r.concurrentUsers <= 100)
            .length,
      ...
      expect(loadSummary.avgSuccessRate).toBeGreaterThan(0.8); // 80% average success rate
      ...
      const performanceGrade =
        loadSummary.avgSuccessRate > 0.9 &&
        loadSummary.avgThroughputRPS > 100 &&
        loadSummary.avgResponseTime < 500
          ? 'A'
          : loadSummary.avgSuccessRate > 0.85 &&
              loadSummary.avgThroughputRPS > 75 &&
              loadSummary.avgResponseTime < 800
            ? 'B'
            : loadSummary.avgSuccessRate > 0.8 &&
                loadSummary.avgThroughputRPS > 50 &&
                loadSummary.avgResponseTime < 1200
              ? 'C'
              : 'D';
