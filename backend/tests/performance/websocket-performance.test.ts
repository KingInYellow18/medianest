...
describe('WebSocket Connection Performance Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const userPromises = Array(50).fill(null).map((_, index) =>
      authHelper.createTestUser(`wstest${index}@medianest.test`)
    );
  ...
  afterAll(async () => {
    ...
    const avgThroughput = wsBenchmarks.reduce((sum, b) => sum + b.throughputMPS, 0) / wsBenchmarks.length;
  ...
  const analyzeWebSocketPerformance = (
    ...
    const totalMessages = metrics.filter(m => m.operation === 'message' || m.operation === 'broadcast').length;
    ...
    const benchmark: WebSocketBenchmark = {
      ...
      throughputMPS: totalMessages > 0 ? Math.round((totalMessages / (Math.max(...metrics.map(m => m.timestamp)) - Math.min(...metrics.map(m => m.timestamp)))) * 1000) : 0,
      memoryEfficiency: Math.round(metrics.reduce((sum, m) => sum + m.memoryUsage.heapUsed, 0) / metrics.length / (1024 * 1024) * 100) / 100,
      passed: false
    };
  ...
  describe('WebSocket Connection Establishment', () => {
    ...
    test('should establish 10 concurrent connections efficiently', async () => {
      ...
      const benchmark = analyzeWebSocketPerformance('concurrent-connections-10', connectionMetrics, 200);
    ...
    test('should handle 50 concurrent connections under load', async () => {
      ...
      for (let batch = 0; batch < batches; batch++) {
        ...
        const successfulMetrics = batchMetrics
          .filter((result): result is PromiseFulfilledResult<WebSocketMetric> => result.status === 'fulfilled')
  ...
  describe('WebSocket Message Performance', () => {
    ...
    test('should handle message broadcasting efficiently', async () => {
      ...
      for (let i = 0; i < connectionCount; i++) {
        ...
        const userIndex = i % testUsers.length;
        try {
          const socket = await createWebSocketClient(testUsers[userIndex].id, userTokens[userIndex], connectionId);
  ...
  describe('WebSocket Performance Summary', () => {
    test('should meet overall WebSocket performance requirements', async () => {
      const wsSummary = {
        ...
        avgOperationTime: Math.round(wsBenchmarks.reduce((sum, b) => sum + b.avgOperationTime, 0) / wsBenchmarks.length),
        avgSuccessRate: wsBenchmarks.reduce((sum, b) => sum + b.successRate, 0) / wsBenchmarks.length,
        avgThroughputMPS: Math.round(wsBenchmarks.filter(b => b.throughputMPS > 0).reduce((sum, b) => sum + b.throughputMPS, 0) / wsBenchmarks.filter(b => b.throughputMPS > 0).length),
        avgMemoryEfficiency: Math.round(wsBenchmarks.reduce((sum, b) => sum + b.memoryEfficiency, 0) / wsBenchmarks.length * 100) / 100,
        totalMetrics: wsMetrics.length,
        operationBreakdown: {
      ...
      expect(wsSummary.avgSuccessRate).toBeGreaterThan(0.90); // 90% operation success rate
      ...
      const performanceGrade = 
        wsSummary.avgSuccessRate > 0.95 && wsSummary.avgOperationTime < 100 && wsSummary.avgThroughputMPS > 50 ? 'A' :
        wsSummary.avgSuccessRate > 0.90 && wsSummary.avgOperationTime < 150 && wsSummary.avgThroughputMPS > 30 ? 'B' :
        wsSummary.avgSuccessRate > 0.85 && wsSummary.avgOperationTime < 200 && wsSummary.avgThroughputMPS > 20 ? 'C' : 'D';
