    ...
    test('should meet overall file throughput performance standards', async () => {
      const overallStats = {
        ...
        avgThroughputMBps: Math.round(
          (performanceMetrics.reduce((sum, m) => sum + m.throughput, 0) / performanceMetrics.length) / (1024 * 1024) * 100
        ) / 100,
        ...
        avgMemoryEfficiency: performanceMetrics.reduce((sum, m) => 
          sum + (m.memoryUsage.heapUsed / m.fileSize), 0) / performanceMetrics.length,
        operationsBySize: {
          small: performanceMetrics.filter(m => m.fileSize < 50 * 1024).length,
          medium: performanceMetrics.filter(m => m.fileSize >= 50 * 1024 && m.fileSize < 500 * 1024).length,
