...
describe('JWT Utilities', () => {
  describe('generateToken', () => {
    ...
    it('should generate a regular token with 24 hour expiry', () => {
      const payload = { userId: 'user123', email: 'user@example.com', role: 'user' }
      const token = generateToken(payload, false)
      ...
      // 24 hours in seconds
      expect(expiryTime).toBe(24 * 60 * 60)
    })
  ...
  describe('verifyToken', () => {
    ...
    it('should throw error for tampered token', () => {
      const token = generateToken({ userId: 'user123', email: 'user@example.com', role: 'user' }, false)
