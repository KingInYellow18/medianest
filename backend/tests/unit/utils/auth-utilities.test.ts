...
describe('Authentication Utilities Unit Tests', () => {
  describe('JWT Token Management', () => {
    ...
    describe('generateToken', () => {
      ...
      it('should generate different tokens for same payload (due to random IDs)', () => {
        const token1 = generateToken(mockPayload);
        const token2 = generateToken(mockPayload);

        // JWT tokens include random sessionId and jti, so they will be different
        // even with same payload
        expect(token1).not.toBe(token2);

        // But they should decode to similar payload structure
    ...
    describe('verifyToken', () => {
      ...
      it('should throw AppError for token with wrong audience', () => {
        const jwt = require('jsonwebtoken');
        const wrongAudienceToken = jwt.sign(mockPayload, process.env.JWT_SECRET, {
    ...
    describe('decodeToken', () => {
      ...
      it('should decode expired token without throwing', () => {
        const jwt = require('jsonwebtoken');
        const expiredToken = jwt.sign(mockPayload, 'any-secret', { expiresIn: '-1h' });
    ...
    describe('getTokenExpiry', () => {
      ...
      it('should handle expired tokens', () => {
        const jwt = require('jsonwebtoken');
        const expiredToken = jwt.sign(mockPayload, process.env.JWT_SECRET, { expiresIn: '-1h' });
    ...
    describe('isTokenExpired', () => {
      ...
      it('should handle edge case of just-expired token', () => {
        const jwt = require('jsonwebtoken');
        const almostExpiredToken = jwt.sign(mockPayload, process.env.JWT_SECRET, {
          expiresIn: '1ms',
        });
    ...
    describe('bcrypt operations', () => {
      ...
      it('should handle various salt rounds', async () => {
        ...
        for (const saltRounds of rounds) {
          ...
          expect(hash).toMatch(
            new RegExp(`^\\$2[aby]?\\$${saltRounds.toString().padStart(2, '0')}\\$`),
          );
        }
      });
  ...
  describe('Security Properties', () => {
    ...
    it('should use secure defaults for JWT', () => {
      ...
      expect(decoded.payload.iss).toBe('medianest'); // Actual default from jwt.ts
      expect(decoded.payload.aud).toBe('medianest-users'); // Actual default from jwt.ts
    });
