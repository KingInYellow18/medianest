...
interface IsolationIssue {
  file: string;
  line: number;
  issue: 'missing_beforeEach' | 'missing_afterEach' | 'shared_state' | 'mock_leakage' | 'no_cleanup';
...
export class TestIsolationAnalyzer {
  ...
  analyzeFile(filePath: string): IsolationIssue[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (/^\s*(let|var|const)\s+\w+.*=.*(?!vi\.fn|Mock)/.test(line) && 
  ...
  private applyUniversalIsolationPattern(content: string, filePath: string): string {
    ...
    for (const line of lines) {
      if (inImportSection && (line.startsWith('import ') || line.startsWith('export ') || line.trim() === '')) {
        imports.push(line);
      } else {
    ...
    const isolationImport = "import { setupUniversalTestIsolation } from './mocks/foundation/universal-test-isolation';";
