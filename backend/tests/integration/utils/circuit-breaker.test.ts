...
describe('Circuit Breaker Integration Tests', () => {
  ...
  describe('Error Classification', () => {
    ...
    it('should handle mixed expected and unexpected errors', async () => {
      const errors = [
        ...
        new Error('Internal server error'), // Unexpected - should count (1st failure)
        new Error('timeout'), // Expected - should not count
        new Error('Database connection failed'), // Unexpected - should count (2nd failure)
        new Error('Service unavailable'), // Unexpected - should count (3rd failure)
        new Error('Another server error'), // Unexpected - but circuit should be open by now
      ];
      ...
      for (const error of errors) {
        ...
        // Count expected failures (errors that don't match expected patterns)
        const errorMessage = error.message.toLowerCase();
        const isExpectedError = ['econnrefused', 'enotfound', 'timeout'].some((expected) =>
        ...
        // Circuit should open after 3 unexpected failures
        if (expectedFailureCount >= 3) {
          break;
        }
      }

      const stats = circuitBreaker.getStats();
      expect(stats.state).toBe(CircuitBreakerState.OPEN); // Should be open after 3 unexpected errors
      expect(stats.failures).toBe(3); // Should count only unexpected errors
    });
  });
  ...
  describe('Timing and Concurrency', () => {
    it('should handle concurrent operations correctly', async () => {
      mockOperation.mockImplementation(async () => {
        // Use Promise.resolve() instead of setTimeout for fake timers compatibility
    ...
    it('should handle mixed concurrent success and failure', async () => {
      ...
      const stats = circuitBreaker.getStats();
      expect(stats.state).toBe(CircuitBreakerState.OPEN); // Should be open after 3 failures
    });
  ...
  describe('Configuration Validation', () => {
    it('should work with different failure thresholds', async () => {
      ...
      expect(lowThresholdBreaker.getStats().state).toBe(CircuitBreakerState.OPEN);
    });
    ...
    it('should work with different reset timeouts', async () => {
      ...
      expect(fastResetBreaker.getStats().state).toBe(CircuitBreakerState.OPEN);
      ...
      expect(result).toBe('success');
      expect(fastResetBreaker.getStats().state).toBe(CircuitBreakerState.CLOSED);
    });
    ...
    it('should handle empty expected errors list', async () => {
      ...
      expect(noExpectedErrorsBreaker.getStats().state).toBe(CircuitBreakerState.OPEN);
    });
  });
});
