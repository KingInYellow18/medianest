...
describe('Media API Integration Tests', () => {
  ...
  describe('GET /api/v1/media/requests', () => {
    ...
    test('should filter requests by status', async () => {
      ...
      await prisma.mediaRequest.createMany({
        data: [
          ...
          { title: 'Rejected Movie', year: 2020, type: 'movie', tmdbId: 1003, userId: user.id, status: 'rejected' }
        ]
      });
    ...
    test('should sort requests by date', async () => {
      ...
      await prisma.mediaRequest.createMany({
        data: [
          { title: 'Old Movie', year: 2020, type: 'movie', tmdbId: 1001, userId: user.id, requestedAt: new Date(now.getTime() - 86400000) },
          { title: 'New Movie', year: 2020, type: 'movie', tmdbId: 1002, userId: user.id, requestedAt: now }
        ]
      });
  ...
  describe('Performance and Load Tests', () => {
    test('should handle high volume of search requests', async () => {
      ...
      const searchPromises = Array(50).fill(null).map((_, index) =>
        request(app)
          .get('/api/v1/media/search')
          .query({ 
            query: `test${index}`,
    ...
    test('should maintain response times under concurrent load', async () => {
      const users = await Promise.all(
        Array(10).fill(null).map(() => authHelper.createUserWithTokens())
      );
