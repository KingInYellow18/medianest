...
describe('Health and Monitoring API Integration Tests', () => {
  ...
  describe('GET /api/v1/health/metrics', () => {
    test('should require admin authentication', async () => {
      ...
      // User role (not admin)
      const { accessToken: userToken } = await authHelper.createUserWithTokens('user@test.com', 'USER');
      await request(app)
        .get('/api/v1/health/metrics')
  ...
  describe('Error Handling and Resilience', () => {
    test('should handle multiple service failures gracefully', async () => {
      ...
      const offlineServices = response.body.data.services.filter((s: any) => s.status === 'offline');
    ...
    test('should handle timeout scenarios', async () => {
      ...
      vi.doMock('../../src/services/health.service', () => ({
        healthService: {
          checkDatabaseHealth: vi.fn().mockImplementation(() => 
