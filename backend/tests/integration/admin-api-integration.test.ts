...
describe('Admin API Integration Tests', () => {
    ...
    test('should get users list with pagination', async () => {
      ...
      await Promise.all(
        Array(15).fill(null).map((_, index) =>
          authHelper.createTestUser(`user${index}@test.com`, 'USER')
        )
      );
  ...
  describe('PATCH /api/v1/admin/users/:userId/role', () => {
    ...
    test('should prevent self-demotion', async () => {
      const { user, accessToken } = await authHelper.createUserWithTokens('admin@test.com', 'ADMIN');
  ...
  describe('GET /api/v1/admin/requests', () => {
    test('should get all media requests with admin view', async () => {
      ...
      await prisma.mediaRequest.createMany({
        data: [
          { title: 'User 1 Movie', year: 2020, type: 'movie', tmdbId: 1001, userId: user1.id, status: 'pending' },
          { title: 'User 2 Movie', year: 2020, type: 'movie', tmdbId: 1002, userId: user2.id, status: 'approved' }
        ]
      });
    ...
    test('should filter requests by status for admin', async () => {
      ...
      await prisma.mediaRequest.createMany({
        data: [
          { title: 'Pending Movie', year: 2020, type: 'movie', tmdbId: 1001, userId: user.id, status: 'pending' },
          { title: 'Approved Movie', year: 2020, type: 'movie', tmdbId: 1002, userId: user.id, status: 'approved' }
        ]
      });
  ...
  describe('GET /api/v1/admin/stats', () => {
    test('should get comprehensive system statistics', async () => {
      ...
      await prisma.mediaRequest.createMany({
        data: [
          { title: 'Movie 1', year: 2020, type: 'movie', tmdbId: 1001, userId: user.id, status: 'pending' },
          { title: 'Movie 2', year: 2020, type: 'movie', tmdbId: 1002, userId: user.id, status: 'approved' }
        ]
      });
  ...
  describe('Admin Security Tests', () => {
    ...
    test('should rate limit admin endpoints', async () => {
      ...
      const responses = await Promise.all(requests.map(req => 
        req.then(res => res.status).catch(() => 429)
      ));
