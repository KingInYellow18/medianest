...
describe('YouTube Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    app.get('/api/v1/youtube/health', (req, res) => {
      res.json({ status: 'ok' });
    });
  });
  ...
  it('should have youtube endpoints available', async () => {
    const response = await request(app).get('/api/v1/youtube/health').expect(200);
