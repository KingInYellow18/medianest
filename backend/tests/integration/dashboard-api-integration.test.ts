...
describe('Dashboard API Integration Tests', () => {
  ...
  describe('GET /api/v1/dashboard/status/:service', () => {
    ...
    test('should handle service check timeouts', async () => {
      ...
      vi.doMock('../../src/services/health.service', () => ({
        healthService: {
          checkServiceHealth: vi.fn().mockImplementation(() =>
  ...
  describe('GET /api/v1/dashboard/notifications', () => {
    ...
    test('should support pagination for notifications', async () => {
      ...
      const notifications = Array(25).fill(null).map((_, index) => ({
  ...
  describe('Performance and Optimization Tests', () => {
    ...
    test('should maintain performance with large datasets', async () => {
      ...
      const mediaRequests = Array(1000).fill(null).map((_, index) => ({
        title: `Movie ${index}`,
        year: 2000 + (index % 23),
        type: 'movie',
        tmdbId: 10000 + index,
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle service timeouts gracefully', async () => {
      ...
      vi.doMock('../../src/services/dashboard.service', () => ({
        dashboardService: {
          getServiceStatuses: vi.fn().mockImplementation(() =>
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('ETIMEDOUT')), 100)
            )
          )
