...
export const overseerrHandlers = [
  ...
  // Search multi
  http.get('*/api/v1/search/multi', ({ request }) => {
  ...
  // Movie details
  http.get('*/api/v1/movie/:movieId', ({ params }) => {
  ...
  // TV show details
  http.get('*/api/v1/tv/:tvId', ({ params }) => {
    const { tvId } = params;
    ...
    return HttpResponse.json({ error: 'TV show not found' }, { status: 404 });
  }),
