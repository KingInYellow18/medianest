...
import { z } from 'zod';
import { secureUrlSchema } from '../src/validations/common';
...
describe('Security Audit - Input Validation', () => {
  ...
  it('should validate URL inputs properly', () => {
    // Test the secure URL schema that rejects dangerous protocols
    const validUrls = [
      'https://youtube.com/watch?v=123',
      'http://localhost:3000',
      'https://example.com/path/to/resource',
      'ftp://example.com', // FTP is allowed in our secure schema
    ];
    ...
    validUrls.forEach((url) => {
      ...
      if (!result.success) {
        console.log(`Valid URL "${url}" failed validation:`, result.error.format());
      }
      expect(result.success).toBe(true);
    });
    ...
    invalidUrls.forEach((url) => {
      const result = secureUrlSchema.safeParse(url);
      if (result.success) {
        console.log(`Invalid URL "${url}" passed validation but should have failed`);
      }
