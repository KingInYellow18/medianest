...
async function globalTeardown(config: FullConfig) {
  ...
  try {
    ...
    try {
      execSync('test -d tests/e2e/reports/allure-results && npx allure generate tests/e2e/reports/allure-results -o tests/e2e/reports/allure-report --clean', {
        stdio: 'inherit',
        timeout: 60000,
      });
    ...
    if (!process.env.CI) {
      ...
      try {
        execSync('docker-compose -f docker-compose.e2e.yml exec -T postgres-e2e psql -U e2e_user -d medianest_e2e -c "DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;"', {
          timeout: 30000,
        });
      } catch (error) {
