...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    if (!process.env.CI) {
      ...
      execSync(
        'docker-compose -f docker-compose.e2e.yml exec -T postgres-e2e pg_isready -U e2e_user -d medianest_e2e',
        {
          timeout: 60000,
        },
      );
    ...
    const setupInfo = {
      ...
      ci: !!process.env.CI,
      browserProjects:
        config.projects?.filter((p) => p.use?.headless !== undefined).map((p) => p.name) || [],
    };
