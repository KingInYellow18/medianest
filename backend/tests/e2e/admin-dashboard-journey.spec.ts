...
import { AdminDashboardPage, AdminUsersPage, AdminServicesPage, AdminStatsPage, AdminBroadcastPage } from '../tests/e2e/pages/admin.page';
...
test.describe('Admin Dashboard User Journey', () => {
  ...
  test('should monitor service health', async () => {
    ...
    await expect(redisCard.locator('[data-testid="warning-message"]')).toContainText('High memory usage');
  ...
  test('should view detailed service information', async () => {
    ...
    await expect(modal.locator('[data-testid="service-url"]')).toContainText('plex.medianest.local:32400');
  });
  ...
  test('should manage media requests', async () => {
    ...
    await expect(pendingRequest.locator('[data-testid="request-title"]')).toContainText('Inception');
    ...
    // Verify success
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Request approved successfully');
  });
  ...
  test('should reject media requests with reason', async () => {
    ...
    // Verify success
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Request rejected successfully');
  });
  ...
  test.describe('Error Handling and Edge Cases', () => {
    test('should handle service monitoring errors gracefully', async () => {
      ...
      await expect(page.locator('[data-testid="services-error"]')).toContainText('monitoring temporarily unavailable');
    ...
    test('should handle user management errors', async () => {
      ...
      await expect(page.locator('[data-testid="users-error"]')).toContainText('Database connection failed');
    });
  });
