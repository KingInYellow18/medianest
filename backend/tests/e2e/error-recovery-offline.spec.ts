...
test.describe('Error Recovery and Offline Scenarios', () => {
  ...
  test.describe('Network Failure Recovery', () => {
    test('should handle complete network failure gracefully', async () => {
      ...
      await expect(page.locator('[data-testid="offline-message"]')).toContainText(/offline|network/i);
  ...
  test.describe('Service Unavailability Scenarios', () => {
    test('should handle Plex server unavailability', async () => {
      ...
      await expect(page.locator('[data-testid="service-error-message"]')).toContainText('Plex server unavailable');
    ...
    test('should handle YouTube service failures gracefully', async () => {
      ...
      await expect(page.locator('[data-testid="service-error"]')).toContainText(/temporarily unavailable/i);
  ...
  test.describe('Session Management and Recovery', () => {
    test('should handle session expiration during activity', async () => {
      ...
      await expect(page.locator('[data-testid="redirect-after-login"]')).toHaveAttribute('value', '/requests');
    });
    ...
    test('should handle concurrent session conflicts', async () => {
      ...
      await expect(page.locator('[data-testid="conflict-message"]')).toContainText(/another device/i);
  ...
  test.describe('Data Corruption and Recovery', () => {
    test('should handle malformed API responses', async () => {
      ...
      await expect(page.locator('[data-testid="error-message"]')).toContainText(/unable to process/i);
      ...
      await page.route('**/api/v1/media/requests', async (route) => {
        ...
        if (requestCount === 1) {
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              requests: [
                { id: '1', title: 'Movie 1', status: 'pending', type: 'movie', createdAt: '2024-01-01T00:00:00Z' },
                { id: '2', title: 'Movie 2', status: 'approved', type: 'movie', createdAt: '2024-01-02T00:00:00Z' },
                { id: '3', title: 'Movie 3', status: 'completed', type: 'movie', createdAt: '2024-01-03T00:00:00Z' }
              ],
              totalCount: 3
            })
          });
