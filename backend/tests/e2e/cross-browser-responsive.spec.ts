...
test.describe('Mobile Device Compatibility Tests', () => {
  mobileConfigs.forEach((config) => {
    test.describe(`${config.name} Mobile Tests`, () => {
      ...
      test(`should display readable text and accessible buttons on ${config.name}`, async ({
        page,
      }) => {
        const authHelper = new AuthHelper(page);
        await authHelper.loginWithPlex();
...
test.describe('Custom Viewport Responsive Tests', () => {
  customViewports.forEach((viewport) => {
    test(`should adapt layout correctly at ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      page,
    }) => {
...
test.describe('Visual Regression Testing', () => {
  test('should maintain consistent visual appearance across browsers', async ({
    page,
    browserName,
  }) => {
    const authHelper = new AuthHelper(page);
