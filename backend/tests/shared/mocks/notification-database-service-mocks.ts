...
export const createMockNotificationDatabaseService = () => {
  ...
  const service = {
    ...
    createNotification: vi.fn().mockImplementation(
      async (input: CreateNotificationInput): Promise<NotificationRecord> => {
    ...
    getPendingNotifications: vi.fn().mockImplementation(
      async (userId: string): Promise<NotificationRecord[]> => {
        const now = new Date();
    ...
    markAllNotificationsRead: vi.fn().mockImplementation(
    ...
    dismissNotification: vi.fn().mockImplementation(
      async (notificationId: string, userId: string): Promise<NotificationRecord | null> => {
        const notification = notificationStore.get(notificationId);
    ...
    getNotificationStats: vi.fn().mockImplementation(
      async (userId: string): Promise<NotificationStats> => {
        const now = new Date();
        ...
        const recentCount = userNotifications.filter(n => 
    ...
    cleanupExpiredNotifications: vi.fn().mockImplementation(
    ...
    cleanupOldNotifications: vi.fn().mockImplementation(
      async (olderThanDays: number = 30): Promise<number> => {
    ...
    createSystemNotification: vi.fn().mockImplementation(
    ...
    getSystemNotifications: vi.fn().mockImplementation(
    ...
    createBulkNotifications: vi.fn().mockImplementation(
      async (
        userIds: string[],
...
export const createNotificationTestScenarios = () => ({
  ...
  mediaRequestApproved: createMockNotificationInput({
    ...
    actions: [
      { label: 'View Request', action: 'view_request', style: 'primary' },
    ],
  }),
});
