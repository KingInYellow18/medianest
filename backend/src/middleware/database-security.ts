...
export const setDatabaseSecurityHeaders = () => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.databaseSecurity?.validated) {
      res.setHeader(
        'X-Database-Security-Status',
        req.databaseSecurity.secure ? 'secure' : 'issues',
      );
...
export const enforceProductionDatabaseSecurity = () => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.databaseSecurity?.suspiciousQuery) {
      ...
      throw new AppError('SUSPICIOUS_QUERY_BLOCKED', 'Request blocked for security reasons', 400);
    }
