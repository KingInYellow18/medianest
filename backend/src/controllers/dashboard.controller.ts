...
export class DashboardController {
  async getServiceStatuses(req: Request, res: Response) {
    try {
      ...
      // Cache service statuses separately for admins and regular users
      const cacheKey = isAdmin ? 'service:statuses:admin' : 'service:statuses:public';
      const statuses = await cacheService.getOrSet(
        cacheKey,
        () => statusService.getAllStatuses(isAdmin),
        300, // 5 minutes
      );
      ...
      throw new AppError('INTERNAL_ERROR', 'Failed to retrieve service statuses', 500);
    }
  }
  ...
  async getServiceStatus(req: Request, res: Response) {
    try {
      ...
      if (!status) {
        throw new AppError('NOT_FOUND', `Service '${service}' not found`, 404);
      }
      ...
      throw new AppError('INTERNAL_ERROR', 'Failed to retrieve service status', 500);
    }
  }
  ...
  async getNotifications(_req: Request, res: Response) {
    try {
      ...
      throw new AppError('INTERNAL_ERROR', 'Failed to retrieve notifications', 500);
    }
  }
}
