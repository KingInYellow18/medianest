 ...
 */
router.get('/container-stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      try {
        ...
        const memoryLimit = execSync(`cat /sys/fs/cgroup/memory/memory.limit_in_bytes 2>/dev/null || echo "0"`, 
          { encoding: 'utf8' }).trim();
        const cpuQuota = execSync(`cat /sys/fs/cgroup/cpu/cpu.cfs_quota_us 2>/dev/null || echo "-1"`, 
          { encoding: 'utf8' }).trim();
        const cpuPeriod = execSync(`cat /sys/fs/cgroup/cpu/cpu.cfs_period_us 2>/dev/null || echo "100000"`, 
          { encoding: 'utf8' }).trim();

        if (parseInt(memoryLimit) > 0) {
          stats.limits.memory = parseInt(memoryLimit);
        }
        
        if (parseInt(cpuQuota) > 0) {
          stats.limits.cpu = parseInt(cpuQuota) / parseInt(cpuPeriod);
        }
      } catch (error) {
