...
class SecurityValidator {
  ...
  async validateEnvironmentSecurity() {
    ...
    this.results.environment.score = Math.min(this.results.environment.score, this.results.environment.max);
    
    console.log(`  📊 Environment Security Score: ${this.results.environment.score}/${this.results.environment.max}\n`);
  }
  ...
  async validateContainerSecurity() {
    ...
    console.log(`  📊 Container Security Score: ${this.results.container.score}/${this.results.container.max}\n`);
  }
  ...
  async validateNetworkSecurity() {
    ...
    try {
      ...
      const composeFiles = ['docker-compose.yml', 'docker-compose.prod.yml', 'docker-compose.e2e.yml'];
    ...
    console.log(`  📊 Network Security Score: ${this.results.network.score}/${this.results.network.max}\n`);
  }
  ...
  async validateAuthenticationSecurity() {
    ...
    try {
      ...
      const middlewareFiles = fs.readdirSync('src/middleware', { withFileTypes: true })
    ...
    console.log(`  📊 Authentication Security Score: ${this.results.authentication.score}/${this.results.authentication.max}\n`);
  }
  ...
  async validateDatabaseSecurity() {
    ...
    console.log(`  📊 Database Security Score: ${this.results.database.score}/${this.results.database.max}\n`);
  }
  ...
  generateReport() {
    ...
    console.log(`🌍 Environment Security:  ${this.results.environment.score}/${this.results.environment.max} (${Math.round(this.results.environment.score/this.results.environment.max*100)}%)`);
    console.log(`🐳 Container Security:    ${this.results.container.score}/${this.results.container.max} (${Math.round(this.results.container.score/this.results.container.max*100)}%)`);
    console.log(`🌐 Network Security:      ${this.results.network.score}/${this.results.network.max} (${Math.round(this.results.network.score/this.results.network.max*100)}%)`);
    console.log(`🔐 Authentication:        ${this.results.authentication.score}/${this.results.authentication.max} (${Math.round(this.results.authentication.score/this.results.authentication.max*100)}%)`);
    console.log(`🗃️ Database Security:     ${this.results.database.score}/${this.results.database.max} (${Math.round(this.results.database.score/this.results.database.max*100)}%)`);
    
    console.log('\n' + '='.repeat(40));
    console.log(`📊 OVERALL SECURITY SCORE: ${overall.score}/${overall.max} (${Math.round(overall.percentage)}%)`);
  ...
  generateRecommendations() {
    ...
    if (this.results.environment.score < 8) {
      ...
      recommendations.push('Implement proper secret management (HashiCorp Vault, AWS Secrets Manager)');
    }
