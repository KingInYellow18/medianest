...
class DisasterRecoveryValidator {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.metrics.rtoAchieved && result.metrics.rtoAchieved > scenario.rtoTarget) {
            console.log(
              `⚠️  RTO target missed: ${result.metrics.rtoAchieved}min > ${scenario.rtoTarget}min`,
            );
          }
          if (result.metrics.rpoAchieved && result.metrics.rpoAchieved > scenario.rpoTarget) {
            console.log(
              `⚠️  RPO target missed: ${result.metrics.rpoAchieved}min > ${scenario.rpoTarget}min`,
            );
          }
        } else {
  ...
  private async testContainerCrashRecovery(): Promise<TestResult> {
    ...
    try {
      ...
      const containerStatus = execSync(
        'docker ps --filter "name=medianest-backend-prod" --format "{{.Status}}"',
        {
          encoding: 'utf-8',
        },
      ).trim();
      ...
      if (containerStatus.includes('Up')) {
        ...
        if (newStatus.includes('Up')) {
          ...
          execSync('docker-compose -f docker-compose.production.yml up -d backend', {
            encoding: 'utf-8',
          });
        }
      } else {
  ...
  private async testRedisFailureRecovery(): Promise<TestResult> {
    ...
    try {
      ...
      // Set a test key
      execSync(
        'docker exec medianest-redis-prod redis-cli SET disaster-test "recovery-validation"',
        {
          encoding: 'utf-8',
        },
      );
  ...
  private async testDiskSpaceExhaustion(): Promise<TestResult> {
    ...
    try {
      ...
      const logConfig = execSync(
        'docker inspect medianest-backend-prod --format "{{.HostConfig.LogConfig}}"',
        {
          encoding: 'utf-8',
        },
      );
