...
interface ServiceCardProps extends OptimizedProps<{
  service: Service;
...
const ServiceMetrics = memo(({ 
  ...
  const uptimePercentage = useMemo(() => 
    Math.round(uptime * 100 * 100) / 100, [uptime]
  );
...
const ServiceCard = memo<ServiceCardProps>(({ 
  ...
  const cardClasses = useMemo(() => {
    ...
    return [baseClasses, statusClass, priorityClass, className]
      .filter(Boolean)
      .join(' ');
  ...
  const renderMetadata = useMemo((): RenderMetadata => ({
...
export const withLazyServiceCard = <P extends object>(
  ...
  const LazyServiceCard = memo((props: P & ServiceCardProps & { 
    loading?: boolean; 
  }) => {
