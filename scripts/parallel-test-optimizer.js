...
class HighPerformanceTestRunner {
  ...
  printPerformanceReport(results) {
    ...
    console.log(`💾 Peak memory: ${(this.performanceMetrics.memoryPeak / 1024 / 1024).toFixed(2)}MB`);
    console.log(`⚡ Average speed: ${(this.performanceMetrics.totalTests / (totalDuration / 1000)).toFixed(2)} tests/sec`);
...
Examples:
  ...
  if (workerOverride) {
    const workers = parseInt(workerOverride.split('=')[1] || args[args.indexOf(workerOverride) + 1]);
