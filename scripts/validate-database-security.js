...
class DatabaseSecurityValidator {
  ...
  validatePostgreSQL() {
    ...
    if (databaseUrl) {
      ...
      if (isProduction) {
        ...
        const hasPoolConfig = databaseUrl.includes('connection_limit') || databaseUrl.includes('pool_timeout');
        this.warn(
  ...
  validateRedis() {
    ...
    if (isProduction) {
      const hasAuth = !!(redisPassword || (redisUrl && redisUrl.includes(':') && redisUrl.split(':')[2]));
      this.test(
    ...
    if (redisUrl) {
      try {
        ...
        this.test(
          'Redis URL Parsing',
          false,
          `Invalid REDIS_URL format: ${error.message}`
        );
      }
    }
  }
  ...
  async runValidation() {
    ...
    console.log(`${colors.bold}${colors.blue}🛡️  MediaNest Database Security Validation${colors.reset}\n`);
    ...
    const successRate = this.results.tests > 0 ? Math.round((this.results.passed / this.results.tests) * 100) : 0;
    
    if (this.results.failed === 0) {
      console.log(`${colors.green}${colors.bold}✅ Security validation completed successfully! (${successRate}% success rate)${colors.reset}`);
      
      if (this.results.warnings > 0) {
        console.log(`${colors.yellow}ℹ️  ${this.results.warnings} warnings found - review recommendations above${colors.reset}`);
      }
      
      return true;
    } else {
      console.log(`${colors.red}${colors.bold}❌ Security validation failed with ${this.results.failed} critical issues${colors.reset}`);
      console.log(`${colors.yellow}Please address the failed tests above before deploying to production${colors.reset}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  validator.runValidation()
    ...
    .catch(error => {
      console.error(`${colors.red}${colors.bold}💥 Validation error: ${error.message}${colors.reset}`);
