...
class MediaNestPerformanceProfiler {
  ...
  async runPerformanceAnalysis() {
    ...
    console.log(`⏱️  Test Duration: ${this.testDuration/1000}s | Concurrent Users: ${this.concurrentUsers}`);
  ...
  async performCpuProfiling() {
    ...
    return {
      ...
      recommendation: userCpuUsage > 10 ? 
  ...
  analyzeResponseTimes(requests) {
    ...
    return {
      ...
      withinThreshold: {
        p95: (durations[Math.floor(durations.length * 0.95)] || 0) <= this.thresholds.api.responseTime.p95,
        p99: (durations[Math.floor(durations.length * 0.99)] || 0) <= this.thresholds.api.responseTime.p99
      }
    };
  }
  ...
  async simulateCpuIntensiveOperations() {
    ...
    const promises = operations.map(op => new Promise(resolve => {
      const result = op();
      resolve(result);
    }));
  ...
  async testFileUploadPerformance() {
    ...
    for (const file of testFiles) {
      try {
        ...
        await new Promise(resolve => setTimeout(resolve, file.size / (10 * 1024 * 1024) * 1000)); // Simulate network time
    ...
    const averageThroughput = results
      .filter(r => !r.error)
      .reduce((sum, r) => sum + r.throughputBytesPerSecond, 0) / results.filter(r => !r.error).length;
  ...
  async analyzeStorageIO() {
    ...
    const avgWriteThroughput = results
      .filter(r => !r.error)
      .reduce((sum, r) => sum + r.writeThroughput, 0) / results.filter(r => !r.error).length;
    
    const avgReadThroughput = results
      .filter(r => !r.error)
      .reduce((sum, r) => sum + r.readThroughput, 0) / results.filter(r => !r.error).length;
    ...
    return {
      ...
      meetsThreshold: Math.min(avgWriteThroughput, avgReadThroughput) >= this.thresholds.fileProcessing.ioThroughput
    };
  }
  ...
  async analyzeAssets(assets) {
    const imageAssets = assets.filter(a => ['.jpg', '.jpeg', '.png', '.gif', '.webp'].includes(a.type));
    const jsAssets = assets.filter(a => ['.js', '.mjs'].includes(a.type));
    const cssAssets = assets.filter(a => ['.css'].includes(a.type));
    const otherAssets = assets.filter(a => !imageAssets.includes(a) && !jsAssets.includes(a) && !cssAssets.includes(a));
    ...
    return {
      ...
      breakdown: {
        images: { count: imageAssets.length, sizeBytes: imageSize, sizeMB: imageSize / (1024 * 1024) },
        ...
        other: { count: otherAssets.length, sizeBytes: otherAssets.reduce((sum, a) => sum + a.size, 0) }
      },
  ...
  generateAssetRecommendations(assets, totalSize) {
    ...
    const largeImages = assets.filter(a => ['.jpg', '.jpeg', '.png'].includes(a.type) && a.size > 500 * 1024);
    if (largeImages.length > 0) {
      recommendations.push(`Consider compressing ${largeImages.length} large images (${largeImages.map(i => i.name).join(', ')})`);
    }
  ...
  generateBundleRecommendations(totalSize, chunks) {
    ...
    if (totalSize > this.thresholds.frontend.bundleSize) {
      recommendations.push(`Bundle size (${(totalSize / (1024 * 1024)).toFixed(2)}MB) exceeds recommended threshold of ${(this.thresholds.frontend.bundleSize / (1024 * 1024)).toFixed(2)}MB`);
    }
    
    // Check for large individual chunks
    const largeChunks = chunks.filter(c => c.size > 500 * 1024); // > 500KB
    if (largeChunks.length > 0) {
      recommendations.push(`${largeChunks.length} chunks are larger than 500KB. Consider code splitting.`);
    }
    
    // Check for missing vendor bundle
    const hasVendorBundle = chunks.some(c => c.isVendor);
    if (!hasVendorBundle) {
      recommendations.push('Consider separating vendor libraries into a dedicated chunk for better caching');
    }
    
    // Check main bundle size
    const mainBundle = chunks.find(c => c.isMainBundle);
    if (mainBundle && mainBundle.size > 250 * 1024) { // > 250KB
  ...
  generateMarkdownSummary() {
    const analysis = this.results.analysis || { criticalIssues: [], warnings: [], optimizations: [] };
...
`;
  ...
  printPerformanceSummary(report) {
    ...
    console.log('\n📈 KEY METRICS:');
    console.log(`   Memory Usage: ${this.results.nodeJsPerformance.memoryAnalysis?.finalUsage?.heapUsed ? (this.results.nodeJsPerformance.memoryAnalysis.finalUsage.heapUsed / (1024*1024)).toFixed(2) : 'N/A'}MB`);
    console.log(`   API P95 Response: ${this.results.apiPerformance.responseTimeAnalysis?.p95?.toFixed(2) || 'N/A'}ms`);
    console.log(`   Bundle Size: ${this.results.frontendPerformance.bundleOptimization?.summary?.totalSizeMB?.toFixed(2) || 'N/A'}MB`);
    console.log(`   Error Rate: ${((this.results.apiPerformance.errorRateMonitoring?.errorRate || 0) * 100).toFixed(2)}%`);
...
if (require.main === module) {
  ...
  profiler.runPerformanceAnalysis()
    .then(report => {
      profiler.printPerformanceSummary(report);
      
      // Exit with appropriate code
      const hasIssues = report.analysis.criticalIssues.length > 0 || 
                       report.analysis.warnings.length > 2;
