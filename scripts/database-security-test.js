...
class DatabaseSecurityTester {
  ...
  log(level, message, data = null) {
    ...
    console.log(
      `${color}${colors.bold}[${level.toUpperCase()}]${colors.reset} ${timestamp} - ${message}`,
    );
    ...
    await this.runTest('PostgreSQL SSL Configuration Validation', async () => {
      ...
      for (const testConfig of testConfigs) {
        ...
        try {
          ...
          this.log(
            'info',
            `✓ ${testConfig.name}: ${hasCriticalIssues ? 'Failed as expected' : 'Passed as expected'}`,
          );
        } finally {
  ...
  async testRedisSecurity() {
    await this.runTest('Redis Security Configuration Validation', async () => {
      ...
      for (const testConfig of testConfigs) {
        ...
        try {
          const results = this.securityManager.validateDatabaseSecurity(testConfig.config);
          const hasCriticalIssues = results.redis.issues.some(
    ...
    await this.runTest('Security Issue Detection', async () => {
      ...
      for (const issue of issues) {
        ...
        try {
          ...
          const allIssues = [...results.postgresql.issues, ...results.redis.issues];
  ...
  async testPerformance() {
    await this.runTest('Security Validation Performance', async () => {
      ...
      if (avgTime > 50) {
        // 50ms per validation is reasonable
        throw new Error(`Security validation too slow: ${avgTime.toFixed(2)}ms average`);
      }

      this.log(
        'info',
        `✓ Performance test: ${avgTime.toFixed(2)}ms average over ${iterations} iterations`,
      );
    });
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  tester.runAllTests().catch((error) => {
    console.error(
      `${colors.red}${colors.bold}💥 Test runner error: ${error.message}${colors.reset}`,
    );
