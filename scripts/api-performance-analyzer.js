...
class APIPerformanceAnalyzer {
  ...
  async loadTestingPhase() {
    ...
    const progressInterval = setInterval(() => {
      ...
      process.stdout.write(
        `\r📊 Progress: ${progress.toFixed(1)}% | Requests: ${requestCount} | Errors: ${errorCount} | RPS: ${rps.toFixed(1)}`,
      );
    }, 5000);
  ...
  analyzeResponseTimes() {
    ...
    const distribution = {
      ...
      good: durations.filter(
        (d) => d >= this.thresholds.responseTime.excellent && d < this.thresholds.responseTime.good,
      ).length,
      acceptable: durations.filter(
        (d) =>
          d >= this.thresholds.responseTime.good && d < this.thresholds.responseTime.acceptable,
      ).length,
      poor: durations.filter(
        (d) =>
          d >= this.thresholds.responseTime.acceptable && d < this.thresholds.responseTime.poor,
      ).length,
  ...
  analyzeThroughput() {
    ...
    const throughputStats = {
      overall:
        this.results.requests.length / ((this.results.endTime - this.results.startTime) / 1000),
      windows: timeWindows,
  ...
  calculateThroughputWindows(windowSize) {
    ...
    for (let windowStart = startTime; windowStart < endTime; windowStart += windowSize) {
      ...
      windows.push({
        ...
        averageResponseTime:
          requestsInWindow.length > 0
            ? requestsInWindow.reduce((sum, r) => sum + r.duration, 0) / requestsInWindow.length
            : 0,
      });
    }
  ...
  calculateThroughputStability(windows) {
    ...
    const variance =
      rpsValues.reduce((sum, rps) => sum + Math.pow(rps - mean, 2), 0) / rpsValues.length;
  ...
  analyzeEndpointPerformance() {
    ...
    this.endpoints.forEach((endpoint) => {
      ...
      const requests = this.results.requests.filter(
        (r) => r.endpoint === endpoint.path && r.method === endpoint.method,
      );
      const errors = this.results.errors.filter(
        (e) => e.endpoint === endpoint.path && e.method === endpoint.method,
      );
      ...
      if (requests.length > 0) {
        ...
        endpointStats[key] = {
          ...
          averageResponseSize:
            requests.reduce((sum, r) => sum + r.responseSize, 0) / requests.length,
          rating: this.rateEndpointPerformance(
            durations,
            errors.length / (requests.length + errors.length),
            endpoint.critical,
          ),
        };
      } else {
  ...
  rateEndpointPerformance(durations, errorRate, isCritical) {
    ...
    const threshold = isCritical
      ? this.thresholds.responseTime.good
      : this.thresholds.responseTime.acceptable;
  ...
  calculatePerformanceRating(responseTime, throughput, errors) {
    ...
    if (
      ...
      responseTime.statistics.p95 > this.thresholds.responseTime.critical
    ) {
      score -= 40;
    } else if (
      responseTime.statistics &&
      responseTime.statistics.p95 > this.thresholds.responseTime.poor
    ) {
      score -= 30;
    } else if (
      responseTime.statistics &&
      responseTime.statistics.p95 > this.thresholds.responseTime.acceptable
    ) {
      score -= 20;
    } else if (
      responseTime.statistics &&
      responseTime.statistics.p95 > this.thresholds.responseTime.good
    ) {
      score -= 10;
    }
  ...
  generateRecommendations(responseTime, throughput, errors) {
    ...
    if (
      ...
      recommendations.push({
        ...
        priority:
          responseTime.statistics.p95 > this.thresholds.responseTime.critical ? 'CRITICAL' : 'HIGH',
...
${Object.values(analysis.endpoints)
  .map(
    (endpoint) =>
...
`;
  ...
  printSummary(report) {
    ...
    console.log(
      `   Success Rate: ${((1 - report.detailedAnalysis.errors.errorRate) * 100).toFixed(2)}%`,
    );
