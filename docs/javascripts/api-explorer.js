...
(function () {
  ...
  class APIExplorer {
    ...
    isAPIExample(content) {
      return (
        content.match(/^(GET|POST|PUT|DELETE|PATCH)\s+\//) ||
    ...
    isAPITable(table) {
      ...
      return headers.some(
        (header) =>
          header.includes('endpoint') || header.includes('method') || header.includes('url'),
      );
    }
                        ...
                        <div class="environment-selector">
                            ...
                            <select class="environment-select">
                                ${Array.from(this.environments.entries())
                                  .map(
                                    ([key, env]) => `<option value="${key}">${env.name}</option>`,
                                  )
                                  .join('')}
                            </select>
                        </div>
...
public class MediaNestAPI {
    ...
    createAPIExplorer() {
      ...
      if (
        ...
        this.isAPIExample(document.querySelector('pre code').textContent)
      ) {
